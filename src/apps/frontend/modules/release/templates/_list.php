...
<?php $release = $releases[$i] ?>
  ...
  <?php endif; ?>
          <img height="220px" width="220px" src="<?php echo $release['image'] ?>" />
            <h1><a data-pjax="#content_async" href="<?php echo url_for(sprintf('@release_show?slug=%s#release', $release['slug'])) ?>"> <?php echo $release['Artist']['name'] ?></a> - <a data-pjax="#content_async" title="<?php echo $release['title'] ?>" href="<?php echo url_for(sprintf('@release_show?slug=%s#release', $release['slug'])) ?>"><?php echo truncate_text($release['title'], 1) ?></a></h1>
